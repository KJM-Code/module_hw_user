...
def register():
    ...
    if form.validate():
        hashed_password = generate_password_hash(form.password.data, method=hashing_type)
...
def change_login_password():
    if current_user.is_authenticated:
        ...
        if form.validate():
            user = database.SYS_USERS.query.filter_by(username=form.username.data).first()
            if check_password_hash(user.password, form.password.data):
                hashed_password_change = generate_password_hash(form.new_password.data, method=hashing_type)
